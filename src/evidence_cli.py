...
CREATE INDEX IF NOT EXISTS idx_weekly_start ON weekly_summary(week_start);
""",
    # v3: artifact-verification cache backing verify.py (the phantom-propagation
    # gate). DDL is mirrored in verify.py CLAIM_TABLE_DDL; both use CREATE TABLE
    # IF NOT EXISTS so whichever runs first wins. Keep the two in sync.
    3: """
...
def _migrate_state_in_memory(state: dict) -> dict:
    ...
    if "verification" not in state or not isinstance(state.get("verification"), dict):
        ...
        ver.setdefault("enabled", True)
        ver.setdefault("escalation_threshold", 3)
        ver.setdefault("ttl_overrides", {})
    # v5: items a carry-forward audit (Phase 3) flags for the next 1:1.
    # Surfaced by valor-prep. Initialized here so readers never KeyError.
    if "escalate_in_one_on_one" not in state or not isinstance(
    ...
    # last_sync_scan); v11 adds meeting_baseline for proactive drift detection
    # (a recurring meeting not in the baseline is a "new project?" signal).
    if "project_focus" not in state or not isinstance(state.get("project_focus"), dict):
        ...
        pf.setdefault("meeting_baseline", [])
...
def cmd_context(args: argparse.Namespace) -> None:
    ...
    result = {
        ...
        "verification": {
            "enabled": bool((state.get("verification") or {}).get("enabled", True)),
