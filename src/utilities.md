...
| `ooo_dates` | Briefing | Out-of-office days (`["YYYY-MM-DD", ...]`). The briefing records full-day `outOfOffice` events it sees on the calendar (today + look-ahead, past dates pruned); `cmd_context` then suppresses the briefing/wrapup/weekly auto-suggestion on those days (`context.ooo.today` / `context.ooo.dates`) and the briefing/wrapup short-circuit. `cmd_context` is offline, so this list is its only OOO signal. The 1:1-prep capacity check also reads `context.ooo.dates`. |
