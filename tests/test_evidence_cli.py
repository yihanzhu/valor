...
def test_ensure_schema_creates_required_tables(cli_db):
    ...
    assert "claim_verifications" in tables  # v3 (verification gate)
    conn.close()
...
def test_claim_verifications_columns(cli_db):
    """The v3 table must carry the columns verify.py depends on."""
    db_path, _ = cli_db
    conn = _make_conn(db_path)
    ensure_schema(conn)
    cols = {row[1] for row in conn.execute(
        "PRAGMA table_info(claim_verifications)"
    ).fetchall()}
    assert {"claim_hash", "verified", "last_checked", "miss_count",
            "day_count", "frozen", "last_verdict", "resolved_at"} <= cols
    conn.close()
    conn.close()
...
def test_migrate_state_in_memory_adds_v5_verification_fields():
    migrated = _migrate_state_in_memory({"current_level": "L3"})
    assert migrated["verification"]["enabled"] is True
...
def test_migrate_state_in_memory_adds_v6_planning_fields():
    ...
    assert migrated["planning"]["block_granularity_minutes"] == 15   # v12
    assert migrated["planning"]["morning_buffer_minutes"] == 0        # v13
    assert migrated["state_schema_version"] == STATE_SCHEMA_VERSION
...
def test_migrate_state_in_memory_preserves_planning_overrides():
    ...
    assert migrated["planning"]["block_granularity_minutes"] == 15   # v12 sub-key filled
    assert migrated["planning"]["morning_buffer_minutes"] == 0        # v13 sub-key filled
...
def test_migrate_state_in_memory_preserves_verification_overrides():
    state = {"verification": {"enabled": False, "ttl_overrides": {"slack": 1}}}
    migrated = _migrate_state_in_memory(state)
    # User's kill switch + overrides survive; only missing sub-keys are filled.
    assert migrated["verification"]["enabled"] is False
    assert migrated["verification"]["ttl_overrides"] == {"slack": 1}
    assert migrated["verification"]["escalation_threshold"] == 3


def test_migrate_state_in_memory_repairs_non_list_escalation():
    migrated = _migrate_state_in_memory({"escalate_in_one_on_one": "garbage"})
    assert migrated["escalate_in_one_on_one"] == []
...
def test_context_includes_verification_block(cli_db, capsys):
    db_path, _ = cli_db
    valor_home = db_path.parent / ".valor"
    (valor_home / "state.json").write_text(json.dumps({
        "verification": {"enabled": False, "escalation_threshold": 5},
        "escalate_in_one_on_one": [{"item": "PROJ-42"}, {"item": "x"}],
    }))
    cmd_context(argparse.Namespace())
    result = json.loads(capsys.readouterr().out)
    assert result["verification"]["enabled"] is False
    assert result["verification"]["escalation_threshold"] == 5
    assert result["escalate_in_one_on_one_count"] == 2


def test_context_verification_defaults_when_absent(cli_db, capsys):
    db_path, _ = cli_db
    valor_home = db_path.parent / ".valor"
